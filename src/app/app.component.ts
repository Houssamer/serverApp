...
export class AppComponent implements OnInit {
  ...
  filterServers(event: Event): void {
    let status = (event.target as HTMLInputElement).value as Status;
  ...
  deleteServer(server: Server): void {
    this.serverService.delete$(server.id).pipe(
      map((response) => {
        this.notifier.onDefault(response.message);
        this.dataSubject.next({
          ...response,
          data: {
            servers: this.dataSubject.value.data.servers.filter((s) => {
              s.id !== server.id;
            }),
          },
        });
  ...
  printReport(): void {
    this.notifier.onDefault('report donwloaded');
